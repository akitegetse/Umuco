...
*    contGenVoltNonRegen.c
*
* Example Category:
*    AO
*
* Description:
*    This example demonstrates how to output a wavefore, not
*    allowing the device to regenerate data from the onboard FIFO.
*    This method requires the user to continuously write data.
*
...
*    7. Loop continuously for 10 seconds.
*       Write new data every 100ms.
...
int main(int argc, char *argv[])
{
    ...
    for(;i<bufferSize;i++)
    {
        ...
        updateData[i] = 2*sin((double)i*2.0*PI/(double)bufferSize);
    }
    ...
    DAQmxErrChk (DAQmxBaseSetWriteAttribute (taskHandle, DAQmx_Write_RegenMode, DAQmx_Val_DoNotAllowRegen));
    ...
    {
        DAQmxErrChk (DAQmxBaseWriteAnalogF64(taskHandle,samplesPerChan,0,timeout,DAQmx_Val_GroupByChannel,updateData,&pointsWritten,NULL));
        if( !done )
            usleep(10000);
    }
